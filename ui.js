...
export function renderFeeds(feeds, removeFeedCallback) {
    ...
    feeds.forEach(feed => {
        ...
        listItem.className = 'flex items-center justify-between bg-white p-3 rounded shadow mb-2 transition-colors duration-200';

        const feedText = document.createElement('span');
        feedText.textContent = feed;
        feedText.className = 'text-gray-700 truncate mr-4';
...
export function renderUpdates(updates) {
    ...
    updateListContainer.innerHTML = sortedUpdates
        .map(update => {
            ...
            return `
            <li class="bg-white p-4 rounded shadow border-l-4 ${update.unread ? 'border-blue-500' : 'border-transparent'} hover:shadow-md transition-shadow duration-200">
                <div class="flex flex-col">
                    <a href="${update.link}" target="_blank" class="text-lg font-medium text-blue-600 hover:underline mb-1">
                        ${update.title}
                    </a>
                    <div class="flex items-center text-sm text-gray-500 space-x-2">
                        ...
                        <span>&bull;</span>
                        <span>${date}</span>
                        ${update.unread ? '<span class="ml-auto text-xs font-bold text-blue-500 bg-blue-100 px-2 py-0.5 rounded-full">NEW</span>' : ''}
                    </div>
                </div>
            </li>
            `;
